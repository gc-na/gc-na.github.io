...
DFT(Design for test)는 Logic의 물리적 결함(아래에서 설명)을 찾기위해 "내부에" 테스트 회로를 추가하는 방법입니다.
...
DFT가 찾는 물리적 결함이 무엇인지 알아보겠습니다.
로직에 문제를 주면 Logic Fault, Delay나 동작속도(Operating speed)에 영향을 주면 물리적 결함입니다.
