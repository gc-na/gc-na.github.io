...
차세대 기억장치들의 저장 방식은 패스 트랜지스터에 축전기 혹은 저항을 연결해서 축전 상태나 저항 상태를 통해 정보를 저장하는 방식입니다.

SCM(Storage Class Memory, 스토리지급 메모리) - 
PRAM(Phase-change RAM, 상변화 메모리) - 
FeRAM(Ferroelectric RAM, 강유전체 메모리) - 
MRAM(Magnetoresistive RAM, 강 자성체 메모리) - 자기성 재료를 이용하여 전원이 없어도 남아있는 자성을 응용하여, 자기저항 정보를 저장하는 장치이다. (HDD나 자기테이프는 자성 정보를 이용한 것)
RRAM(Resistive RAM, 저항메모리. ReRAM으로도 불림.), PoRAM(Polymer RAM, 폴리머 메모리)
