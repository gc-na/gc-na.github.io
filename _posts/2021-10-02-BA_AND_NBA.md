---
layout: post
title: \[Verilog] NBA(NonBlocking Assignment), BA(Blocking Assignment) 차이.
...
erilog를 하면서, 왜 NBA를 쓰는지 모르고, NBA가 단순히 '동시에 실행'이라고 알고 있는 사람들이 많은 것 같아서 정리.
...
Race condition이란? -> 여러개의 코드가 실행 될 때, A가 먼저 실행 되면 A1의 결과. B가 먼저 실행되면 B1의 결과가 나오는데.. 설계자의 의도와 다르게, 어떤 출력이 나올지 알수가 없는 상태.
